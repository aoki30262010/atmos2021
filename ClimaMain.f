...
C   NSOL = # OF SOLAR FREQUENCIES !Aokim compared to Kato+1999
C
C   T = TEMPERATURE (K)
C   P = PRESSURE (bar)
...
!!!!!!!! This version(ver1.1) only CO addition. Aoki2020 !!!!!!!!!!
...
c     PARAMETER(NF=55, NA=1, NLAYERS=51, NZ=64)
      ...
      PARAMETER(NSOL=39, NGS=9, IK=8)  ! Added IK=8 parameter and NGS is 7 now, 3/26/2012
      !gna - changed ngs to 8 (ethane) !change to 9(Aokic)
      parameter(nrow=11)
...
C      CHARACTER*5 :: ICO2A   !Changed to make STARR hold up to 5 characters
      ...
      CHARACTER*11 :: AA
      CHARACTER :: DIRINOUT*8,DIRDATA*10,K2DATA*6

      REAL kappa_solco,kappa_solco2,kappa_solh2o !Aoki2020kd
...
C-jdh DIMENSION LAM(NF),ALAM(NF),AVOLD(NF)
      ...
      DIMENSION FIM(ND-1),MYU(ND),MYUM(ND-1),SIGMAA(ND-1),PCO2T(ND)  !Aoki
      DIMENSION TCHANGE(ND),PCOSIN(ND),PN2SIN(ND),CONVSTA(ND-1)
c     vectors for gaussian zenith angles
      ...
      REAL*8 FCO2init           ! Kurokawa
      REAL*8 DIVK,DIVf,Divfrmso,DelD,SIGCO2,keisu,sigsig1,sigsig7,atai
      REAL*8 maxfla,flafla,seffchg !Aoki2020
...
c     COMMON/CBLOK/FO2,FN2,FCO2,FAR,FCH4
     ...
     &  FCO  ! Added FH2 to CBLOK 5/29/2012 c-rr !Aokic
     ...
     &  NG(2),SIGG(4,2,NSOL)
      COMMON/AOKI/FCT2 !Aoki
      DIMENSION CGAS_CO(ND) !Aoki2020
...
!     & ,kmatrix_sol(NSOL,IK) ! Added SOLARDATA block here 3/26/2012
     ...
     &  TAUCO(NSOL,ND-1),TAUCO2(NSOL,ND-1),TAUH2O(NSOL,ND-1),!Aoki2020
     &  TAUALL(NSOL,ND-1) !Aoki2020
...
c-rr  3/29/11
     ...
     &  CPN(ND), CPNT(ND), CPH2(ND),CPCO(ND)  ! Added CPH2 5/31/2012 c-rr !Aokic
C
      DATA BETA/ND*1./
      DATA BETH2O/1100*0./
      DATA BETCO2/1100*0./
...
C  INPUT FILES
      OPEN (unit=1,file= K2DATA//'/input_clima.dat')
...
!gna: we should update CH4?  There was a big CH4 update in HITRAN 2012
     ...
     &              status='old')  ! CO2 solar coefficients
       ...
       OPEN (unit=401, file=DIRDATA//'/ktable_CO.dat',
     &              status='old')  ! CO solar coefficients Aoki 2020
...
c-as to TempIn.dat in order to start from the last solution, if IUP=0
      ...
      OPEN(UNIT=98,FILE=K2DATA//'/clima_allout.tab') !DIRINOUT to K2DATA !Aoki2020
      OPEN(UNIT=96,FILE= DIRINOUT//'/SolarHeating.tab')
      OPEN(UNIT=97,FILE=K2DATA//'/clima_last.tab') !DIRINOUT to K2DATA !Aoki2020
      OPEN(UNIT=80,FILE= DIRINOUT//'/IR_wavelength_grid.tab')
      OPEN(UNIT=1101,FILE=K2DATA//'/keika.tab') !Aoki2020
      OPEN(UNIT=1102,FILE=K2DATA//'/result_hoken.tab') !Aoki2020
...
!(before this piece of code was before ICOUPLE was read in)
      IF (ICOUPLE.eq.0) THEN
         OPEN (unit=114,file= K2DATA//'/mixing_ratios.dat')
      ELSE
...
c-rr Noncondensible molecular weight of the atmosphere when CO2 is condensing (for a colder planet)          5/3/2011
     ...
     & + 46.*FNO2+ 2.*FH22 +28.*FCO ! c-rr 5/29/2012 added H2 mixing ratio !Aokic
...
c      print *,'GO READSOL'!Aoki2020d

C Read Solar Data
      CALL READSOL
!      print *, kappa_solco(39,8,8,1),kappa_solco2(39,8,8,1) !Aoki2020kd
!     print *, ALAMBDA(1) !Aoki2020d
c      print *,'BACK READSOL'!Aoki2020d
c Choosing a star
...
c Constructing temperature and water profiles in case they are not provided
     ...
     &    IDRY,FLAGCONVEC,FCO)
      ...
      write(fname,'(a)') 't00000.dat'
      open(10,file=fname) !Aoki
      do J=1,ND
      write(10,*)ALT(J),FI(1,J),FSATUR(J),T(J),JCONV,ALT(JCONV)
      end do
      close(10)
...
!      IF(IUP.EQ.1) FI(2,J)=FCO2V(J)
      FI(2,J) = (1. - FI(1,J)) * FCO2init ! Kurokawa
...
C Set up gas concentrations for Solar code and former IR code
      ...
      CALL GASCON(T,PF,FO2,FH22,FI,FNC,CGAS,NST,FCO,CGAS_CO)
! Added FH2 to GASCON input argument 5/30/2012
C SWITCH FOR OLD IR CODE
c      if(FCO2.gt.CO2MAX) then
c-as Old subroutine to calculate IR flux
C PLANCK FUNCTION WAS CHANGED
c      print *, CGAS_co(20)      !Aokic
c      print *, 'BACK GASCON' !Aoki2020d
...
c        if(j.eq.1)print *, 'CPCO2new=', CPCO2(J), T(J)
     ...
     &     + 1.74E-08*T(J)*T(J)*T(J)
      CPCO(J) = 6.1108 + 1.457E-3*T(J) + 9.69E-7*T(J)*T(J)
     &     -6.384E-10*T(J)*T(J)*T(J)+3.13E+4/T(J)/T(J) !Aokic
!      print*,'test',FCO2init,FI(2,J)
...
!      print *, 'CALL SOLAR' !Aoki2020d
        ...
        ELSEIF((IMET.eq.0).and.(IO2.eq.0))THEN
           CALL SOLAR(T,LAST,FNC,NST,CGAS_CO)
        ENDIF
!           print *, kappa_solco(39,8,8,1),kappa_solco2(39,8,8,1) !Aoki2020kd
!        print *, 'END SOLAR' !Aoki2020d

c        print *, 'stage callsolar' !Aoki
c        write(*,*) TAULAM(1)
...
c      write(fname,'(a,i5.5,a)') 't',NST,'.dat'
c      open(10,file=fname) !Aoki
c      do J=1,ND
c      write(10,*)J,FTOTAL(J),FDNSOL(J),FUPSOL(J),FDNIR(J),FUPIR(J)
c      end do
c      close(10)
...
C New temperature calculation for all layers from radiative equilibrum
     ...
     &        /(PF1(J+1)-PF1(J))
      ...
      if(TCHANGE(J) .gt. 10.)then
         TN(J) = T(J) + 10.
      end if
      if(TCHANGE(J) .lt. -10.)then
         TN(J) = T(J) - 10.
      end if
...
c             mixing ratio.
!                   FI(2,J1-1) = FCO2
                     FI(2,J1-1) = (1. - FI(1,J1-1)) * FCO2init ! Kurokawa
                     FLAGCONVEC(J1) = cflag
                   JCONV = J1
           END IF
      END DO
      ...
      ND1 = ND-1
!      DO J=ND1,1,-1
!      FI(2,J) = AMIN1(FI(2,J),FI(2,J+1)) ! Comment outed by Kurokawa.
!      END DO
      GO TO 1401
c
...
c-as   it (ND and ND-1) (sept-2004)
     ...
     & Idry, imco2,FCO)
       ...
       FI(2,ND-1) = (1. - FI(1,ND-1)) * FCO2init ! Kurokawa
...
!          FI(2,J1-1) = FCO2  ! jiggered again
          FI(2,J1-1) = (1.-FI(1,J1-1)) * FCO2init ! Kurokawa, FI1 should be updated first??
        ELSE
            ITROP=0
            FLAGCONVEC(J1)=0.
        ENDIF
...
c         FCT2 = 1.
c      END IF
c     write(*,*)FCT2,FSATUR(ND),PSAT,P(ND) !Aoki
...
C-KK To smooth over the profile around JCOLD.
      ...
      FI(1,JCOLD) = sum/5.
      FI(2,JCOLD) = (1.-FI(1,JCOLD)) * FCO2init ! Kurokawa
...
c Adjusting the time stepper
       ...
       DO J=2,ND-1
        ...
        CHG = AMAX1(CHG,REL)
       END DO
       IF (CHG.LT.0.01) dt0 = DTS * 1.5 !1
       IF (CHG.LT.0.001) dt0 = DTS * 5. !5
       IF (CHG.GT.0.02) dt0 = DTS / 2. !2
...
!     Aoki's section(Pco2)
      ...
      do J=1,ND
         MYU(J)=FI(2,J)*44.+FSAVE(J)*18.+(1.-FI(2,J)-FSAVE(J))*28.
      end do

      SIGCO2 = 0.

      do J=1,ND-1
         FIM(J)=(FI(2,J)+FI(2,J+1))/2.
         MYUM(J)=(MYU(J)+MYU(J+1))/2.
         SIGMAA(J)=FIM(J)*(P(J+1)-P(J))/(MYUM(J)*9.8)
         SIGCO2=SIGCO2 + SIGMAA(J)
         PCO2T(J+1)=44.*9.8*SIGCO2
      end do


  !    write(fname,'(a,i5.5,a)') 't',NST,'.dat'
  !    open(10,file=fname) !Aoki
  !    do J=1,ND
  !       write(10,*)ALT(J),RELHUM(J),FI(1,J),T(J),
  !   &         PCO2T(J),FI(1,JCOLD),FSATUR(J),JCOLD,JCONV
  !    end do
  !$    close(10)

! watch convevtion stability to determine convergent condition (Aoki2020)
      do J=1,ND-1
        if (T(J+1)-T(J) .gt. 0)THEN
          CONVSTA(J)=1
        ELSE
          CONVSTA(J)=0
        end if
      ...
      if (mod(NST,30) .eq. 0) THEN
        write(fname,'(a,i4.4,a)') 'd',NST,'.dat'
            open(10,file=fname) !Aoki
            do J=1,ND
               write(10,*)ALT(J),FI(1,J),T(J),DIVF(J),
     &                    FTOTAL(J),FTSO(J),FTIR(J)
            end do
            close(10)

!        write(fname,'(a)') 'keika.dat'
!            OPEN(10,FILE=K2DATA//fname)
!            open(10,file=fname) !Aoki
!             write(10,*) NST,JCONV,CHG,dt0,DIVF(1),DIVFrms,DELT(ND)
!            close(10)
      end if
      write(1101,*) NST,JCONV,CHG,dt0,DIVF(1),DIVFrms,DELT(ND),T(ND) !keika_tab !Aoki2020
      write(1102,*) NST,T(ND),PCO2T(ND),FI(1,JCOLD),FSATUR(ND),ALBP
!      write(fname,'(a,i5.5,a)') 'seff.dat'
!      OPEN(10,FILE= K2DATA//fname,position='append')
!      open(10,file=fname,position='append')
      !add "position='append'" , not rewrite.
!      write(10,*) NST,seff
!      close(10)
...
C JK Calculate root-mean-square flux divergence above the tropopause
       ...
       JC1 = JCONV-1
       DO J=1,JC1
         ! DIVFrmso = DIVFrms       !Aoki
          DIVFrms = DIVFrms + DIVF(J)*DIVF(J)
         ! if (j .ge. 2)then  !Aoki
         !     delD = log10(DivFrmso) - log10(DivFrms) !Aoki
         ! END If
       END DO
...
C
       ...
       Do j=1,ND-1
         flafla = divf(j)
         if(CONVSTA(j) .eq. 1)THEN
           maxfla = max(maxfla,flafla)
!           if(maxfla .eq. divf(j))THEN
!             print *, j, maxfla
!           end if
         end if
       end do

       write(fname,'(a,i5.5,a)') 'divf.dat'
       open(10,file=fname,position='append')
       !add "position='append'" , not rewrite.
       write(10,*) NST,DIVFrms
       close(10)

c      if(nst.gt.2 .and. nst.lt.nsteps2) then
       WRITE(98,966) NST,JCONV,CHG,dt0,DIVF(1),DIVFrms,DELT(ND),T(ND),
     & FSATUR(ND)
 966   FORMAT(1x,I6,1X,I3,1x,1pe8.2
     & ,1x,1pe8.2,1X,1PE9.2
     & ,1X,1PE9.2,1x,1PE9.2,1x,
     & 1PE10.4,1x,1pe9.2)
c       ENDIF
...
c     & DELT(J),TOLD(J),FI(1,J),HEATNET(J),TCOOL(J),THEAT(J)
     ...
     & DELT(J),TOLD(J),FI(1,J),FSAVE(J),FI(4,J),TCOOL(J),THEAT(J),OMEGA !Aoki
       ENDDO
      WRITE(98,1683)
      Write (98,1112)
 1112 FORMAT(/1x,'FCO2')
 ...
 1111 FORMAT(1x,1p10e9.2)
  ...
  683  FORMAT(/2x,"J",5X,"P",9X,"ALT",9X,"T",8X,"CONVEC",
     ...
     &  7x,'FSAVE',8x,'FO3',8x,'TCOOL',7x,'THEAT',8x,'OMEGA') ! top of file Aoki
...
!      write(*,*) T(ND),PCO2T(ND),FI(1,JCOLD),FSATUR(ND),FSATUR(JCOLD)
!     &        ,ALBP             !Aoki
!      if abs(keisu*100 - 100)<1 then
!         write(*,*) 'OK'
!      endif
      write(*,*) keisu*PG0,100*keisu
!     end Aoki's section

!      write(fname,'(a)') 'tau.dat'
!      open(10,file=fname) !Aoki
!      do J=1,ND-1
!        write(10,*) ALT(J),TAUALL(J),TAUCO(J),TAUCO2(J),TAUH2O(J)  !Aoki2020
!      end do
!      close(10)

      write(fname,'(a)') 'gascon.dat'
      open(10,file=fname) !Aoki
      do J=1,ND
        write(10,*) J,CGAS_CO(J)  !Aoki2020
      end do
      close(10)
...
! This section is carried out in each loop. Aoki2020
 ...
 !*********************************************************
      !Aoki2020
      do NSTE = 1,NSOL
       write(fname,'(a,i2.2,a)') 'tau',NSTE,'.dat'
       OPEN(10,FILE= K2DATA//fname)
!       open(10,file=fname) !Aoki
        do J=2,ND-1
        write(10,*) ALT(J),TAUALL(NSTE,J),TAUCO(NSTE,J),TAUCO2(NSTE,J),
     &               TAUH2O(NSTE,J)  !Aoki2020
        end do
       close(10)
      end do


      write(*,*) T(ND),PCO2T(ND),FI(1,JCOLD),FSATUR(ND),ALBP             !Aoki
      write(*,*) PN2SIN(ND) , PCOSIN(ND)
      ...
      if(ICOUPLE.eq.1) then
       ...
       CALL OUTPUT_PHOTO(T, FI, water, ALT, nzp)
        ...
        WRITE(97,466)
c 466  FORMAT(5X,'ALT',10X,'P',10X,'T',10X,'FH2O',11X,'O3',11X,
c     2  'THEAT',8X,'TCOOL',8X,'PSATCO2',8x,'FCO2')
 466  FORMAT(5X,'ALT',10X,'P',10X,'T',10X,'FH2O',11X,'FCO2',11X,
     2  'FH2O',10x,'PCO2',10x,'FSATUR(ND)')
...
c     &  TCOOL(J)
c        WRITE(97,467) ALT(J),P(J),T(J),FSAVE(J),FI(4,J),THEAT(J),
c     &       TCOOL(J),PSATCO2(J),FI(2,J),FI(1,JCOLD) !comment outed by Aoki
        WRITE(97,467) ALT(J),P(J),T(J),FSAVE(J),
     &       FI(2,J),FI(1,JCOLD),PCO2T(J),FSATUR(J),RELHUM(P(J)) !Aoki

c        WRITE(97,467) ALT(J),FSAVE(J),FI(2,J),(1.-FSAVE(J))*0.92874d0
c     &  TCOOL(J),PSATCO2(J),FI(2,J)
