...
C-jdh DIMENSION LAM(NF),ALAM(NF),AVOLD(NF)
      ...
      DIMENSION TCHANGE(ND),PCOSIN(ND),PN2SIN(ND),RAMDAA(NSOL)!Aoki
c     vectors for gaussian zenith angles      
      ...
      REAL*8 DIVK,DIVf,Divfrmso,DelD,SIGCO2,keisu,sigsig1,sigsig7,atai     !Aoki
...
c     COMMON/CBLOK/FO2,FN2,FCO2,FAR,FCH4
     ...
     &  NG(2),SIGG(4,2,NSOL)
      COMMON/AOKI/FCT2,CGAS_CO(ND) !Aoki
...
c     2  ' zy =',f5.2,' weightt=',f8.5)
      ...
      print *, 'CALL SOLAR' !Aoki2020d
        ...
        ELSEIF((IMET.eq.0).and.(IO2.eq.0))THEN
           CALL SOLAR(T,LAST,FNC,NST,sigsig1,sigsig7)
        ENDIF
...
c Adjusting the time stepper
       ...
       DO J=2,ND-1
         ...
         CHG = AMAX1(CHG,REL)
       END DO
       IF (CHG.LT.0.01) dt0 = DTS*1.5 !1.5
...
C JK Calculate root-mean-square flux divergence above the tropopause
       ...
       DO J=1,JC1
         ! DIVFrmso = DIVFrms       !Aoki
          DIVFrms = DIVFrms + DIVF(J)*DIVF(J) !Aoki
         ! if (j .ge. 2)then  !Aoki
...
c      if(nst.gt.2 .and. nst.lt.nsteps2) then   
 ...
 966   FORMAT(1x,"NST=",I6,1X,'JCONV=',I3,1x,'CHG=',1pe8.2,1x,"dt0=",
     ...
     & "DT(ND)=",1PE9.2,1x,"T(ND)=",1PE10.4,1x,"FSATUR(ND)=",
     & 1pe9.2)   
c       ENDIF
