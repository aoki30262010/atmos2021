...
C   Top of atmosphere
      ...
      B(1) = E1(1)
      D(1) = -E2(1)
c      print *, 'DELTA stageB'!Aoki
C   Odd coefficients
      DO 3 N=1,NZM1
         L = 2*N + 1
c         print *, 'DELTA stageX'!Aoki
      A(L) = E2(N)*E3(N) - E4(N)*E1(N)  !KOITU GA SINDA Aoki
c      print *, 'DELTA stageY'!Aoki
      B(L) = E1(N)*E1(N+1) - E3(N)*E3(N+1)
c      print *, 'DELTA stageD'!Aoki
   3  D(L) = E3(N)*E4(N+1) - E1(N)*E2(N+1)
c        print *, 'DELTA stageE'!Aoki
C   Even coefficients
      DO 4 N=1,NZM1
      L = 2*N
