      SUBROUTINE GASCON(T,PF,FO2,FH2,FI,FNC,CGAS,NST,FCO,CGAS_CO)
      INCLUDE 'CLIMA/INCLUDE/header.inc'
      PARAMETER(NS=3, NS1=NS+2,NGS2=9) ! NGS2 to differentiate from NGS 5/30/2012
      !gna - changed ngas2 to 8 (ethane added) and NS1 to NS+2 !Aokic
...
C  this subroutine is: AIR, CH4,O2, O3, CO2, H2O, and H2. 
     ...
     2  CGAS(ND,NGS2),FNC(ND),CO_val(ND),FF_CO(ND),CGAS_CO(ND)  ! Added noncondensible mixing fraction array c-rr 5/14/2011 
...
!       pause
       ...
       CO_val(ND) = FCO*FNC(J) !CO Aoki2020
  5    CONTINUE
      ...
      FF_co(j) = sqrt(CO_val(j)*CO_val(j-1))
  3   CONTINUE
C

      DO 4 I=1,NGS2          !change 8 to 9 spcies Aoki2020
...
!     Use Simpson's rule for more accuracy on mixing ratios
      DO J = 1,ND1
      CGAS_CO(J) = FACT*(PF(J+1)-PF(J))*1.013*(FF_co(J)+4.*co_val(j)  ! Pressure is in bars converted to atm cuz Loschmidt # units are in molecules/(cm^3*atm).       
     2  + co_val(J+1))/(6.*DM) 
      END DO
      print *,'END GASCON'!Aoki2020d
