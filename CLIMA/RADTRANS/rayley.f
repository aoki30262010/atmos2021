...
C
      ...
      PARAMETER (NSOL=39, NGS=8) !gna ngas changed to 8 !38 to 39 Aoki2020
...
c     COMMON/CBLOK/FO2,FN2,FCO2,FAR,FCH4
     ...
     &   FH22,FCO
      ...
      DIMENSION A(5),B(5),DEL(7),SIG(7)  ! Changed dimension statement so that A,B,and SIG have 4 entries. SIG,DEL has 5,6 for water methane and water, respectively. SIGR is just a scalar. 5/3/2011 !6 to 7 Aokic
C

      REAL SIGR, AIRSCAT, NM,RM, WN, KCF, SIGH,sigsig1,sigsig7
      DATA A/29.06,26.63,43.9,27.92,32.7/ !A(5)=CO Aokic
      DATA B/7.7,5.07,6.4,5.6,8.1/ !B(5)=CO Aokic
      DATA DEL/.0305,.054,.0805,0.032,0.0002,0.17,0.08/ !DEL(7)=CO Aokic
...
C           c-rr 5/29/2011
         ...
         ! Then, and since the term (A(J)*1E-5)^2 is of order ~1E-7, it is multiplied by 4.557E-21, resulting in the 4.55E-28 term.
            PAREN=(1.E-5*A(5)*(1.+1.E-3*B(5)/AL2))**2
            SMDEL=(6.+3.*DEL(7))/(6.-7.*DEL(7))
            SIG(7)=4.577E-21*SMDEL*PAREN/AL4
...
c         print *, Fwater,SIG(6) , FCO2,SIG(3) , FNCR,AIRSCAT !Aoki2020d
...
c     print *,SIG(1),SIG(7),SIG(7)/SIG(1)
c       print *, AL
      RETURN
      END  
