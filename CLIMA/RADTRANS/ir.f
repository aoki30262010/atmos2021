...
c     & xkappa(8,12,55,8,3)
     ...
     4 TAULAMIR(ND-1),TAUAEXTIR(ND-1),TAUASIR(ND-1),TAUSIR(ND-1)
      ...
      REAL kmatrix_irh2o, kmatrix_irco2 ! EWS - KAPPAC2H6(NF), kappa_ir variables not used
      REAL np
...
c         print *, 'IR stage11'!Aoki
      ...
      ENDDO
c     PRINT*, T,P
c      print *, 'IR stage12'!Aoki
       DO 99 I=1, ND
        FUPIR(I) = 0.
        FDNIR(I) = 0.
 99   CONTINUE
c      print *, 'IR stage13'!Aoki
       DO I=1,55
         TAUTOTAL(I) = 0.0
       ENDDO
...
c      print *, 'IR stage15'!Aoki
****** Loop over frequency      
       DO 1 I=1, NF
                   AL2   =  (1E4*(C/AV(I)))**2   ! wavenumber 
                   VAC = AV(I)
c                   print *, 'IR stage16'!Aoki
       DO 20 J=1,ND 
C-jdh  VAC = C*AV(I)
...
c       print *, 'IR stage19'!Aoki        
       ENDDO
       IF (I.EQ.15) THEN
c          print *, 'IR stage20'!Aoki
       TAUEXTIRTOTAL = 0.
       TAUASIRTOTAL = 0.
       DO IL=1,NLAYERS
...
c               print *, 'IR stage24'!Aoki
               ANUM = (exp((C2*AAV)/296.)-1.)*(exp((C2*AAV)/T(IL))+1.)
c               print *, 'IR stage25'!Aoki
...
!---------------------------------------------------------           
c                  write(*,*) self_absir(1,1) !Aoki
c                  write(*,*) s_abir(1)*exp(TDir(1)*(296.-T(1))) !Aoki
c                  write(*,*)T(1) !Aoki
c            write(*,*)FI(1,1) !Aoki
c                 write(*,*)(PH2O/1.)*(296./T(1))  !Aoki
c                write(*,*)P(1)    !Aoki
                  
                  

c         print *, 'IR stage27'!Aoki
...
!         TAUGCH4(IL) = KAPPALAYER(I,K3,3,IL)*CGAS(IL,2)*2.687E24 !! Activated when methane is on (IMET = 1)
!           if(IL .eq. 1)then
!            if(NST==5)then
...
!----------------------------------------------------------
           ...
           TAUH2H2(IL) = AMAX1(TAUH2H2(IL),1.E-38)
           TAUO2O2(IL) = AMAX1(TAUO2O2(IL),1.E-38)
           TAUH2N2(IL) = AMAX1(TAUH2N2(IL),1.E-38)
...
c      print *, 'IR stageB1'!Aoki
      CALL DELTA2STRIR(SRFALBIR,ASYIR,TAULAMIR,OMG0IR,
     & FUP,FDN,BPLANCK,TAUTOP,I, K1,IL)!KOITU GA YABAI Aoki
